...
each node's `mid` is the majority class.
    ...
    [16]> klass = lambda r: "good" if disty(d,r)<0.4 else "bad"
...
so `landscape`'s pool is the train half (the local `half`
avoids shadowing the `mid` reducer). The total label cost is
one budget — no peeking at test.

    [17]> best = holdout(Data(csv(the.file)))
